...
//      document.getElementById('show').src=newsrc;
//      $('.identific-upload-img2').attr("src",newsrc)
		$('.faburenwu-upload').before('<div class="faburenwu-img-list-box"><img class="faburenwu-img-list-img" src="'+newsrc+'"/><img class="faburenwu-img-list-img-x" src="img/ico_close@2x.png"  onclick="delate(this)" /></div>')
    }
    //建立一個可存取到該file的url
    function getObjectURL(file) {
        var url = null ;
...
//删除照片
$('.faburenwu-img-list-img-x').click(function(){
	$(this).parent().remove();
	console.log("aaaa");
})
...
$(function(){
	var status = true;	
	$('.i_eject-box-box-button').click(function(){
		if(status == false){
			return;
		}
		status = false;
		$('.i_eject-box-box-button').css("width","0.2rem");
		$('.i_eject-box-box-button').html("60")
		var time = 60;
		var timer = setInterval(function(){
			time--;
			$('.i_eject-box-box-button').html(time);
			if(time<=0){
				clearInterval(timer);
				$('.i_eject-box-box-button').css("width","0.8rem");
				setTimeout(function(){
					$('.i_eject-box-box-button').html("获取验证码");
				},300)
				status=true;
			}
		},1000)
	...
	$('.i_eject-box-select-img').click(function(){
		if($('.i_eject-box-select-img').attr("src")==""){
			$('.i_eject-box-select-img').attr("src","img/ico_check@2x.png");
			$('.i_eject-buttom').css("background-color","rgb(62,115,243)");
			$('.i_eject-buttom').css("color","#FFF");

		}else{
			$('.i_eject-box-select-img').attr("src","");
			$('.i_eject-buttom').css("background-color","#ddd");
			$('.i_eject-buttom').css("color","#999");
...
$(function(){
				$('.t_box-number-buttom').click(function(){				
					if($(this).children('.t_box-number-buttom-text').html()=="展开"){
						$(this).children('.t_box-number-buttom-text').html("收起");
						$(this).children('.t_box-number-buttom-img').attr("src","img/ico_more@2xtop.png");
						$(this).parents('.t_box').children('.t_box-context').slideDown();
					}else{
						$(this).children('.t_box-number-buttom-text').html("展开");
						$(this).children('.t_box-number-buttom-img').attr("src","img/ico_more@2xbutton.png");
						$(this).parents('.t_box').children('.t_box-context').slideUp();
					}
